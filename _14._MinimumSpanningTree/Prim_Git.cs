...
namespace _14._MinimumSpanningTree
{
    internal class Prim_Git
    {
        ...
        public static void MinimumSpanningTree(in int[,] graph, in int start, out bool[] known, out int[] cost, out int[] path)
        {
            ...
            // 1. 시작 정점만을 최소신장트리 집합에 포함
            PriorityQueue<Edge_GP, int> edgePQ = new PriorityQueue<Edge_GP, int>();
            known[start] = true;
            cost[start] = 0;
            path[start] = -1;
            ...
            {
                // 2-1. 다음 간선 확인
                Edge_GP nextEdge = edgePQ.Dequeue();
        ...
        private static void AddEdges(in int[,] graph, in PriorityQueue<Edge_GP, int> pq, int vertex)
        {
            for (int i = 0; i < graph.GetLength(0); i++)
            {
                ...
                Edge_GP edge = new Edge_GP(vertex, i, graph[vertex, i]);
                pq.Enqueue(edge, edge.distance);
            }
        }

        private struct Edge_GP
        {
            public int start;
            public int end;
            public int distance;

            public Edge_GP(int start, int end, int distance)
            {
                this.start = start;
                this.end = end;
