...
namespace _14._MinimumSpanningTree
{
    internal class Kruskal_Git
    {
        ...
        public static int[,] MinimumSpanningTree(in int[,] graph)
        {
            ...
            // 1. 그래프 내의 모든 간선의 가중치를 오름차순으로 정렬
            PriorityQueue<Edge_GK, int> edgePQ = new PriorityQueue<Edge_GK, int>();
            for (int start = 0; start < graph.GetLength(0); start++)
            {
                for (int end = 0; end < graph.GetLength(1); end++)
                {
                    ...
                    Edge_GK edge = new Edge_GK(start, end, graph[start, end]);
            ...
            {
                // 2-1. 다음 간선 확인
                Edge_GK nextEdge = edgePQ.Dequeue();
        ...
        private struct Edge_GK
        {
            public int start;
            public int end;
            public int distance;

            public Edge_GK(int start, int end, int distance)
            {
                this.start = start;
                this.end = end;
